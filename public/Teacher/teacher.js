(() => {
  ...
  function validateForm() {
    ...
    if (valid) {
      const deadline = new Date(`${elements.fields.dueDate.value}T${elements.fields.dueTime.value || "23:59"}:00`);
      if (isNaN(deadline.getTime()) || deadline.getTime() <= Date.now()) {
        document.getElementById("fieldDueDate").classList.add("invalid");
        document.getElementById("fieldDueTime").classList.add("invalid");
        showToast("Lỗi ngày hạn", "Ngày và giờ đến hạn phải nằm trong tương lai.", "error");
        valid = false;
      }
    }
